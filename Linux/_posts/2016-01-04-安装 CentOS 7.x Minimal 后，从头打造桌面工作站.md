...
为了使用 GitHub、Gitee 等服务的 SSH keys 功能，利用 ssh-keygen 命令在本地生成 rsa 密钥对。

    $ ssh-keygen -t rsa -C "your_email@example.com"

生成的 rsa 公钥、私钥分别存储在`~/.ssh/id_rsa.pub`和`~/.ssh/id_rsa`中，命令中提供的邮箱`your_email@example.com`用作密钥对的 label。
